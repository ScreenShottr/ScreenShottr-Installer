...
Else
	WScript.Echo "Missing parameters.  String AppPathAndName String ShortcutName Boolean OnStartMenu." & vbCr & vbCr & "  Example cmd.exe CMD  false" & vbCr & vbCr & "  Example %windir%\system32\SnippingTool.exe SnipIt false" 
End If
...
Public Sub PinApplicationToTaskBar(AppPathAndName, ShortcutName, OnStartMenu)
	...
	If(trim(lcase(OnStartMenu)) = "true") then ' pinned to start menu
		HasAlreadyBeenPinnedShortCut = ObjShell.ExpandEnvironmentStrings("%APPDATA%") & "\Microsoft\Internet Explorer\Quick Launch\User Pinned\StartMenu"
	Else
		HasAlreadyBeenPinnedShortCut = ObjShell.ExpandEnvironmentStrings("%APPDATA%") & "\Microsoft\Internet Explorer\Quick Launch\User Pinned\TaskBar"
	End If
